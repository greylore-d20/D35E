...
export class ActorUpdater {
    ...
    async #prepareUpdateData(updated) {
        ...
        } else if (expandedData.system !== null)
            updated = flattenObject(expandedData);
        updated.img = img;
        for (let abl of Object.keys(getProperty(this.actor.system,"abilities"))) {
            if (updated[`data.abilities.${abl}.tempvalue`] === undefined || updated[`data.abilities.${abl}.tempvalue`] === null)
                continue
            if (Array.isArray(updated[`data.abilities.${abl}.tempvalue`])) {
                for (let val of updated[`data.abilities.${abl}.tempvalue`]) {
                    if (updated[`data.abilities.${abl}.value`] !== undefined && parseInt(val) !== updated[`data.abilities.${abl}.value`]) {
                        updated[`data.abilities.${abl}.value`] = parseInt(val);
                        break;
                ...
                updated[`data.abilities.${abl}.value`] = parseInt(updated[`data.abilities.${abl}.tempvalue`]);
            }
        }

        // Make certain variables absolute
        const _absoluteKeys = Object.keys(getProperty(this.actor.system,"abilities")).reduce((arr, abl) => {
        ...
        if (updated[`data.attributes.hp.value`] !== undefined && updated[`data.attributes.hp.value`] !== null) {
            if (parseInt(updated[`data.attributes.hp.value`]) == 0)
                updated[`data.attributes.hp.value`] = 0;
            else {
                if (typeof updated[`data.attributes.hp.value`] === "string") {
                    if (updated[`data.attributes.hp.value`].startsWith('+')) {
                        updated[`data.attributes.hp.value`] = getProperty(this.actor.system,"attributes.hp.value") + parseInt(updated[`data.attributes.hp.value`]);
                    } else if (updated[`data.attributes.hp.value`].startsWith('-')) {
                        if (getProperty(this.actor.system,"attributes.hp.value") > 0) // When we are below 0, we cannot do that
                            updated[`data.attributes.hp.value`] = getProperty(this.actor.system,"attributes.hp.value") + parseInt(updated[`data.attributes.hp.value`]);
                        else
                            updated[`data.attributes.hp.value`] = parseInt(updated[`data.attributes.hp.value`]);
                    } else {
                        updated[`data.attributes.hp.value`] = parseInt(updated[`data.attributes.hp.value`]) || this.actor.system.attributes.hp.value
                    }
                } else {
                    updated[`data.attributes.hp.value`] = parseInt(updated[`data.attributes.hp.value`]) || this.actor.system.attributes.hp.value
                }
        ...
        if (updated[`data.attributes.hp.value`] <= -10) {
            updated[`data.attributes.conditions.dying`] = false;
            updated[`data.attributes.conditions.dead`] = true;
        }else if (updated[`data.attributes.hp.value`] < 0) {
            updated[`data.attributes.conditions.dead`] = false;
            updated[`data.attributes.conditions.dying`] = true;
        } else if (updated[`data.attributes.hp.value`] > 0 && updated[`data.attributes.conditions.dying`]) {
            updated[`data.attributes.conditions.dying`] = false;
        } else if ((updated[`data.attributes.hp.max`] || this.actor.system.attributes.hp.max)  === updated[`data.attributes.hp.nonlethal`]) {
            updated[`data.attributes.conditions.staggered`] = true;
        } else if ((updated[`data.attributes.hp.max`] || this.actor.system.attributes.hp.max) < updated[`data.attributes.hp.nonlethal`]) {
            updated[`data.attributes.conditions.staggered`] = false;
            updated[`data.attributes.conditions.unconscious`] = true;
        }

        if (updated[`data.attributes.conditions.dead`] && !this.actor.system.attributes.conditions.dead) {
            const tokens = this.actor.getActiveTokens();
            const deadEffect = CONFIG.controlIcons.defeated;
            for (let token of tokens) {
                token.toggleEffect(deadEffect, { active: true, overlay: true });
            }
        } else if (!updated[`data.attributes.conditions.dead`] && this.actor.system.attributes.conditions.dead) {
            const tokens = this.actor.getActiveTokens();
            const deadEffect = CONFIG.controlIcons.defeated;
            for (let token of tokens) {
        ...
        if (updated[`data.attributes.conditions.stable`]  && !this.actor.system.attributes.conditions?.stable) {
            updated[`data.attributes.conditions.dying`] = false;
        }

        if (updated[`data.attributes.conditions.dying`]  && !this.actor.system.attributes.conditions?.dying) {
            updated[`data.attributes.conditions.unconscious`] = true;
        }
        if (updated[`data.attributes.conditions.unconscious`]  && !this.actor.system.attributes.conditions?.unconscious) {
            updated[`data.attributes.conditions.helpless`] = true;
        }

        if (updated[`data.attributes.conditions.invisible`] && !this.actor.system.attributes.conditions.invisible) {
            // const tokens = this.actor.getActiveTokens();
            // const deadEffect = CONFIG.controlIcons.visibility;
            // for (let token of tokens) {
            //     token.toggleEffect(deadEffect, { active: true, overlay: true });
            // }
        } else if (!updated[`data.attributes.conditions.invisibility`] && this.actor.system.attributes.conditions.invisibility) {
            const tokens = this.actor.getActiveTokens();
            const deadEffect = CONFIG.controlIcons.visibility;
            for (let token of tokens) {
                token.toggleEffect(deadEffect, { active: false, overlay: true });
        ...
        if (updated[`data.attributes.conditions.banished`] && !this.actor.system.attributes.conditions.banished) {
            const tokens = this.actor.getActiveTokens();
            const deadEffect = 'systems/D35E/icons/actions/magic-gate.svg';
            for (let token of tokens) {
                token.toggleEffect(deadEffect, { active: true, overlay: true });
            }
        } else if (!updated[`data.attributes.conditions.banished`] && this.actor.system.attributes.conditions.banished) {
            const tokens = this.actor.getActiveTokens();
            const deadEffect = 'systems/D35E/icons/actions/magic-gate.svg';
            for (let token of tokens) {
                token.toggleEffect(deadEffect, { active: false, overlay: true });
            ...
            if (i.system.containerId !== undefined && i.system.containerId !== "none") {
                ...
                if (container === undefined || container === null) {
                    ...
                    if (i.system.carried !== container.system.carried) {
                        itemUpdateData["data.carried"] = container.system.carried;
                        hasContainerChanged = true
                    }
                    if (i.system.containerWeightless !== container.system.bagOfHoldingLike) {
                        itemUpdateData["data.containerWeightless"] = container.system.bagOfHoldingLike;
                        hasContainerChanged = true
                    }
                }
            } else {
    ...
    async #updateExp(updated) {
        ...
        if (this.actor.items != null) {
            try {
                ...
                this.actor.items.filter(o => o.type === "class").forEach(c => {
        ...
        let racialHD = classes.filter(o => o.type === "class" && getProperty(o.system,"classType") === "racial").reduce((cur, o) => {
            return cur + o.system.levels;
        }, 0);
        level += raceLA;
        ...
        if (updated["data.details.levelUpProgression"] || getProperty(this.actor.system,"details.levelUpProgression")) {
            dataLevel = (updated["data.details.level.available"] || getProperty(this.actor.system,"details.level.available")) + raceLA + racialHD
        ...
        let newExp = updated["data.details.xp.value"],
            resetExp = false;
    ...
    async updateChanges({updated = null} = {}, options = {}) {
        ...
        const changeObjects = source.items.filter(obj => {
            return obj.system.changes != null;
        }).filter(obj => {
            let z = obj.type;
            if (obj.type === "buff") return obj.system.active;
            if (obj.type === "equipment" || obj.type === "weapon") return (obj.system.equipped && !obj.system.melded && !obj.broken);
            return true;
        });
        ...
        await this.#addDefaultChanges(source, allChanges, flags, sourceInfo, fullConditions, sizeOverride, options, updateData);
        ...
        updateData = flattenObject({ data: mergeObject(origData.system, expandObject(updateData).system, { inplace: false }) });
        ...
        allChanges.forEach((change, a) => {
            ...
            try {
                ...
                ui.notifications.error(game.i18n.localize("D35E.ErrorItemFormula").format(change.source?.item?.name || "Unknown (most likely Actor itself)", this.actor.name, `${formula} (${changeTarget})`));
            }
            ...
            if (allChanges.length <= a + 1 || allChanges[a + 1].raw[2] !== changeTarget) {
                const newData = this.#applyChanges(changeTarget, temp, source, sourceInfo,change.source.name || change.source?.item?.name, change.source.type);
                this.#addDynamicData(updateData, newData, flags, Object.keys(getProperty(this.actor.system,"abilities")), source, false, changeTarget);
                temp = [];
            }
        });
        ...
        for (let flagKey of Object.keys(flags)) {
            ...
            switch (flagKey) {
                case "noDex":
                    ...
                    linkData(source, updateData, `data.abilities.dex.drain`, 0);
                    break;
                case "noStr":
                    ...
                    linkData(source, updateData, `data.abilities.str.drain`, 0);
                    break;
                case "noCon":
                    ...
                    linkData(source, updateData, `data.abilities.con.drain`, 0);
                    break;
                case "noInt":
                    ...
                    linkData(source, updateData, `data.abilities.int.drain`, 0);
                    break;
                case "oneInt":
                ...
                case "oneWis":
                    ...
                    linkData(source, updateData, `data.abilities.wis.drain`, 0);
                    break;
                case "oneCha":
                    ...
                    linkData(source, updateData, `data.abilities.cha.drain`, 0);
                    break;
            }
        }
        ...
        // Reset spell slots
        for (let spellbookKey of Object.keys(getProperty(source, "data.attributes.spells.spellbooks"))) {
            const spellbookAbilityKey = getProperty(source, `data.attributes.spells.spellbooks.${spellbookKey}.ability`);
            const spellslotAbilityKey = getProperty(source, `data.attributes.spells.spellbooks.${spellbookKey}.spellslotAbility`) || spellbookAbilityKey
            let spellbookAbilityMod = getProperty(source, `data.abilities.${spellbookAbilityKey}.mod`);
            let spellslotAbilityMod = getProperty(source, `data.abilities.${spellslotAbilityKey}.mod`);
            const spellbookClass = getProperty(source, `data.attributes.spells.spellbooks.${spellbookKey}.class`);
            const autoSetup = getProperty(source, `data.attributes.spells.spellbooks.${spellbookKey}.autoSetup`);
            let classLevel = getProperty(source, `data.classes.${spellbookClass}.level`) + parseInt(getProperty(source, `data.attributes.spells.spellbooks.${spellbookKey}.bonusPrestigeCl`));
            if (classLevel > getProperty(source, `data.classes.${spellbookClass}.maxLevel`))
                classLevel = getProperty(source, `data.classes.${spellbookClass}.maxLevel`);
            const classProgression = getProperty(source, `data.classes.${spellbookClass}.spellPerLevel${classLevel}`);
            let autoSpellLevels = getProperty(source, `data.attributes.spells.spellbooks.${spellbookKey}.autoSpellLevels`);
            if (autoSetup) {
                const autoSpellcastingAbilityKey = getProperty(source, `data.classes.${spellbookClass}.spellcastingAbility`)
                const autoSpellslotAbilityKey = getProperty(source, `data.classes.${spellbookClass}.spellslotAbility`) || autoSpellcastingAbilityKey
                for (let property of [["spellcastingType", "spellcastingType"], ["ability", "spellcastingAbility"], ["spellslotAbility", "spellslotAbility"], ["spontaneous", "isSpellcastingSpontaneus"]])
                    linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.${property[0]}`, getProperty(source, `data.classes.${spellbookClass}.${property[1]}`));
                linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.autoSpellLevels`, true);
                linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.usePowerPoints`, getProperty(source, `data.classes.${spellbookClass}.isPsionSpellcaster`));
                linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.arcaneSpellFailure`, getProperty(source, `data.classes.${spellbookClass}.isArcane`));
                linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.hasSpecialSlot`, getProperty(source, `data.classes.${spellbookClass}.hasSpecialSlot`));

                autoSpellLevels = true;
                spellbookAbilityMod = getProperty(source, `data.abilities.${autoSpellcastingAbilityKey}.mod`) + getProperty(source, `data.attributes.spells.spellbooks.${spellbookKey}.spellcastingAbilityBonus`)
                spellslotAbilityMod = getProperty(source, `data.abilities.${autoSpellslotAbilityKey}.mod`) + getProperty(source, `data.attributes.spells.spellbooks.${spellbookKey}.spellcastingAbilityBonus`)
            }

            let powerPointsFormula = updateData[`data.attributes.spells.spellbooks.${spellbookKey}.dailyPowerPointsFormula`] || getProperty(source, `attributes.spells.spellbooks.${spellbookKey}.dailyPowerPointsFormula`) || "0"
            linkData(source, updateData,`data.attributes.spells.spellbooks.${spellbookKey}.powerPointsTotal`,new Roll35e(powerPointsFormula, this.actor.getRollData()).roll().total);

            for (let a = 0; a < 10; a++) {
                const classBase = classProgression !== undefined ? parseInt(classProgression[a + 1]) : -1;
                let base = parseInt(getProperty(source, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.base`));
                let bonus = parseInt(getProperty(source, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.bonus`) || 0);
                if (!autoSpellLevels) {
                    if (Number.isNaN(base)) {
                        linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.base`, null);
                        linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.max`, 0);
                    } else {
                        linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.max`, base);
                    }
                } else {
                    if (classBase >= 0) {
                        const value = (typeof spellslotAbilityMod === "number") ? (classBase + this.getSpellSlotIncrease(spellslotAbilityMod, a)) : classBase;
                        linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.base`, value);
                        linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.max`, value + bonus);
                    } else {
                        linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.base`, 0);
                        linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.max`, 0);
                    }
                }
        ...
        // Reset spell slots
        let classes = this.actor.items.filter(o => o.type === "class" && getProperty(o, "classType") !== "racial").sort((a, b) => {
            return a.sort - b.sort;
        });

        for (let deckKey of Object.keys(getProperty(source, "data.attributes.cards.decks"))) {
            const spellbookAbilityKey = getProperty(source, `data.attributes.cards.decks.${deckKey}.ability`);
            const spellslotAbilityKey = getProperty(source, `data.attributes.cards.decks.${deckKey}.spellslotAbility`) || spellbookAbilityKey
            let spellbookAbilityMod = getProperty(source, `data.abilities.${spellbookAbilityKey}.mod`);
            let spellslotAbilityMod = getProperty(source, `data.abilities.${spellslotAbilityKey}.mod`);
            const spellbookClass = getProperty(source, `data.attributes.cards.decks.${deckKey}.class`);
            const autoSetup = getProperty(source, `data.attributes.cards.decks.${deckKey}.autoSetup`);
            const deckAddHalfOtherLevels = getProperty(source, `data.attributes.cards.decks.${deckKey}.addHalfOtherLevels`);
            let baseClassLevel = getProperty(source, `data.classes.${spellbookClass}.level`)
            let classLevel = getProperty(source, `data.classes.${spellbookClass}.level`)  + parseInt(getProperty(source, `data.attributes.cards.decks.${deckKey}.bonusPrestigeCl`) || 0);
            if (classLevel > getProperty(source, `data.classes.${spellbookClass}.maxLevel`))
                classLevel = getProperty(source, `data.classes.${spellbookClass}.maxLevel`);


            linkData(source, updateData,`data.attributes.cards.decks.${deckKey}.cl.base`,classLevel);

            let deckHandSizeForumla = getProperty(source, `data.classes.${spellbookClass}.deckHandSizeFormula`)  || "0";
            let baseDeckHandSize = new Roll35e(deckHandSizeForumla || "", {level: classLevel || 0}).roll().total
            let knownCardsSizeFormula = getProperty(source, `data.classes.${spellbookClass}.knownCardsSizeFormula`)  || "0";
            ...
            let otherClassesLevels = classes.reduce((cur, o) => {
                if (o.name === spellbookClass || o.system.classType === "minion" || o.system.classType === "template" || o.system.deckPresigeClass) return cur;
                return cur + o.system.levels;
            }, 0);
            ...
            let deckSizeFormula = updateData[`data.attributes.cards.decks.${deckKey}.handSize.formula`] || getProperty(source, `attributes.cards.decks.${deckKey}.handSize.formula`) || "0"
            linkData(source, updateData,`data.attributes.cards.decks.${deckKey}.handSize.total`,new Roll35e(deckSizeFormula, this.actor.getRollData()).roll().total + baseDeckHandSize);

            let casterLevelBonusFormula = updateData[`data.attributes.cards.decks.${deckKey}.cl.formula`] || getProperty(source, `attributes.cards.decks.${deckKey}.cl.formula`) || "0"
            linkData(source, updateData,`data.attributes.cards.decks.${deckKey}.cl.total`,new Roll35e(casterLevelBonusFormula, this.actor.getRollData()).roll().total + totalDeckCasterLevel);

            let knownCardsBonusFormula = updateData[`data.attributes.cards.decks.${deckKey}.deckSize.formula`] || getProperty(source, `attributes.cards.decks.${deckKey}.deckSize.formula`) || "0"
            linkData(source, updateData,`data.attributes.cards.decks.${deckKey}.deckSize.total`,new Roll35e(knownCardsBonusFormula, this.actor.getRollData()).roll().total + baseKnownCardsSize);
        }


        // Add dex mod to AC
        if (updateData["data.abilities.dex.mod"] < 0 || !flags.loseDexToAC) {
            const maxDexBonus = updateData["data.attributes.maxDexBonus"] || (getProperty(this.actor.system,"attributes.maxDexBonus") || null);
            ...
            if (!updateData["data.attributes.ac.normal.replace"]) //We did not replace AC, continue
                linkData(source, updateData, "system.attributes.ac.normal.total", updateData["data.attributes.ac.normal.total"] + dexBonus);

            if (!updateData["data.attributes.ac.touch.replace"]) //We did not replace AC, continue
                linkData(source, updateData, "system.attributes.ac.touch.total", updateData["data.attributes.ac.touch.total"] + dexBonus);

            if (!updateData["data.attributes.ac.flatFooted.replace"]) {
                if (updateData["data.abilities.dex.mod"] < 0) {
                    linkData(source, updateData, "system.attributes.ac.flatFooted.total", updateData["data.attributes.ac.flatFooted.total"] + dexBonus);
                }
        ...
        if (fullConditions.wildshaped || fullConditions.polymorph) {
            linkData(source, updateData, "system.attributes.hp.value", Math.min(prevValues.mhp, source.system.attributes.hp.value));
        } else {
            if (updateData["data.attributes.hp.max"]) {
                const hpDiff = updateData["data.attributes.hp.max"] - prevValues.mhp;
                LogHelper.log('D35E | HP Diff', prevValues.mhp, hpDiff, updateData["data.attributes.hp.max"])
                if (hpDiff !== 0) {
                    linkData(source, updateData, "system.attributes.hp.value", Math.min(updateData["data.attributes.hp.max"], source.system.attributes.hp.value + hpDiff));
                }
            }
        }
        if (updateData["data.attributes.wounds.max"]) {
            ...
            if (wDiff !== 0) {
                linkData(source, updateData, "system.attributes.wounds.value", Math.min(updateData["data.attributes.wounds.max"], source.system.attributes.wounds.value + wDiff));
            }
        }
        if (updateData["data.attributes.vigor.max"]) {
            const vDiff = updateData["data.attributes.vigor.max"] - prevValues.vigor;
            if (vDiff !== 0) {
                linkData(source, updateData, "system.attributes.vigor.value", Math.min(updateData["data.attributes.vigor.max"], source.system.attributes.vigor.value + vDiff));
            }
        }
        if (source !== null) {
            if (source.img !== getProperty(this.actor.system,"tokenImg") && getProperty(this.actor.system,"tokenImg") === "icons/svg/mystery-man.svg") {
        ...
        if (!options.skipToken && !getProperty(this.actor.system,"noTokenOverride")) {
            if (shapechangeImg !== "icons/svg/mystery-man.svg") {
                ...
                if (!this.actor.isToken) {
                    ...
                    if (source !== null)
                        source["token.img"] = shapechangeImg;
                }
            } else {
                if (this.actor.isToken) {
                    let tokens = []
        ...
        if (fullConditions.wildshaped || fullConditions.polymorph) //this.actor.retains max HP
        ...
        this.actor.sourceDetails = ActorPrepareSourceHelper.setSourceDetails(mergeObject(this.actor.toObject(false), source), sourceInfo, flags);
    ...
    #updateAbilityRelatedFields(source, updateData, sourceInfo) {

        {
            ...
            let chaMod = getProperty(source, `data.abilities.cha.mod`)
            //LogHelper.log(updateData)
            if (getProperty(source, `data.attributes.turnUndeadHdTotal`) > 0) {
...

        {
            ...
            const classes = source.items.filter(o => o.type === "class" && getProperty(o.system, "classType") !== "racial").sort((a, b) => {
                return a.sort - b.sort;
            });
            const k = "data.attributes.powerPointsTotal";
            linkData(source, updateData, k, updateData[k] + classes.reduce((cur, obj) => {
                try {
                    if (obj.system.powerPointTable === undefined || obj.system.powerPointTable[obj.system.levels] === undefined)
                        return cur
                    let ablMod = 0;
                    if (obj.system.powerPointBonusBaseAbility !== undefined && obj.system.powerPointBonusBaseAbility !== null && obj.system.powerPointBonusBaseAbility !== "")
                        ablMod = getProperty(source, `data.abilities.${obj.system.powerPointBonusBaseAbility}.mod`) || 0;
        ...
        if (source.system.jumpSkillAdjust) {
            ...
            if (source.system.attributes.speed.land.total < source.system.attributes.speed.land.base) {
                value = 6*Math.floor((source.system.attributes.speed.land.total - source.system.attributes.speed.land.base)/10);
                sourceInfo[j].negative.push({ name: "Speed penalty", value: value });
            } else {
                value = 4*Math.floor((source.system.attributes.speed.land.total - source.system.attributes.speed.land.base)/10);
    ...
    async #resetData(updateData, source, flags, sourceInfo, changes, fullConditions) {
        ...
        if (this.actor.items != null) {
            try {
                ...
                if (raceObject != null) {
                    raceLA = raceObject.system.la || 0
                    linkData(source, updateData, "system.attributes.creatureType", getProperty(raceObject.system, "creatureType") || "humanoid");
        ...
        if (racialHD.length > 0) {
            linkData(source, updateData, "system.attributes.creatureType", getProperty(racialHD[0].system, "creatureType") || source.system.attributes.creatureType);
        }
        // Set creature type
        if (templateHD.length > 0) {
            linkData(source, updateData, "system.attributes.creatureType", getProperty(templateHD[0].system, "creatureType") || source.system.attributes.creatureType);
        }
        ...
        for (let [a, abl] of Object.entries(data1.abilities)) {
            linkData(source, updateData, `data.abilities.${a}.penalty`, 0);
            ...
            linkData(source, updateData, `data.abilities.${a}.checkMod`, 0);
            linkData(source, updateData, `data.abilities.${a}.total`, abl.value - Math.abs(abl.drain) - Math.abs(abl.damage));
            linkData(source, updateData, `data.abilities.${a}.mod`, Math.floor((updateData[`data.abilities.${a}.total`] - 10) / 2));
        }

        // Reset maximum hit points
        linkData(source, updateData, "system.attributes.hp.max", getProperty(source, "data.attributes.hp.base") || 0);
        linkData(source, updateData, "system.attributes.wounds.max", getProperty(source, "data.attributes.wounds.base") || 0);
        linkData(source, updateData, "system.attributes.vigor.max", getProperty(source, "data.attributes.vigor.base") || 0);

        // Reset AC
        for (let type of Object.keys(data1.attributes.ac)) {
            linkData(source, updateData, `data.attributes.ac.${type}.total`, 10);
        }
        ...
        for (let currency of currencyConfig.currency) {
            if (currency[0])
                if (data1.attributes.customCurrency === undefined || data1.attributes.customCurrency[currency[0]] === undefined) {
                    linkData(source, updateData, `data.attributes.customCurrency.${currency[0]}`, 0);
                }
        }
        ...
        if (data1.attributes.prestigeCl === undefined) {
            ...
            for (let type of ["psionic", "arcane", "divine", "cards"]) {
                ...
                if (data1.attributes.prestigeCl[type] === undefined) {
                    linkData(source, updateData, `data.attributes.prestigeCl.${type}`, {
                        "max": 0,
                        "value": 0
                    });
                } else {
                    linkData(source, updateData, `data.attributes.prestigeCl.${type}.max`, 0);
                }
            }
        }
        ...
        items.filter(obj => {
            ...
            linkData(source, updateData, "system.attributes.acp.gear", updateData["data.attributes.acp.gear"] + itemAcp);
            ...
            if (getProperty(obj.system,'armor.dex') !== null && getProperty(obj.system,'armor.dex') !== "") {
                if (updateData["data.attributes.maxDexBonus"] == null)  {
                    ...
                    linkData(source, updateData, "system.attributes.maxDexBonus", Math.min(updateData["data.attributes.maxDexBonus"], Math.abs(obj.system.armor.dex)));
                    linkData(source, updateData, "system.attributes.maxDex.gear", Math.min(updateData["data.attributes.maxDexBonus"], Math.abs(obj.system.armor.dex)));
                }
            }
        });
        ...
        for (let speedKey of Object.keys(getProperty(this.actor.system,"attributes.speed"))) {
            let base = getProperty(source, `data.attributes.speed.${speedKey}.base`);
            linkData(source, updateData, `data.attributes.speed.${speedKey}.total`, base || 0);
        }
        ...
        // Reset BAB, CMB and CMD
        {
            ...
            let epicLevels = 0;
            if (useFractionalBaseBonuses) {
                linkData(source, updateData, k, Math.floor(classes.reduce((cur, obj) => {
                ...
                let bab = classes.reduce((cur, obj) => {
                    const formula = CONFIG.D35E.classBABFormulas[obj.system.bab] != null ? CONFIG.D35E.classBABFormulas[obj.system.bab] : "0";
                    let classLevel = obj.system.levels;
        ...
        // Turn undead total level
        {
            ...
            linkData(source, updateData, k, classes.reduce((cur, obj) => {
                try {
...

        {
            ...
            if (getProperty(source, `data.attributes.sr.formula`).length > 0) {
                let roll = new Roll35e(getProperty(source, `data.attributes.sr.formula`), source.system).roll();
        ...
        }
        {
            ...
            // Set spell resistance
            if (getProperty(source, `data.attributes.hardness.formula`).length > 0) {
                let roll = new Roll35e(getProperty(source, `data.attributes.hardness.formula`), source.system).roll();
                linkData(source, updateData, k, roll.total);
            } else {
        ...
        }
        {
            ...
            linkData(source, updateData, k, parseInt(getProperty(source, `data.details.breakDC.base`) || "0"));
        }

        {
            ...
            if (game.settings.get("D35E", "psionicsAreDifferent")) {
                if (getProperty(source, `data.attributes.pr.formula`)?.length > 0) {
                    let roll = new Roll35e(getProperty(source, `data.attributes.pr.formula`), source.system).roll();
                    linkData(source, updateData, k, roll.total);
                } else {
        ...
        // Total sneak attak dice
        {
            ...
            classes.forEach(obj => {
                try {
                    if (obj.system.sneakAttackGroup == null || obj.system.sneakAttackGroup === "")
                        return;
                    ...
                    groupLevels.set(obj.system.sneakAttackGroup, groupLevels.get(obj.system.sneakAttackGroup) + obj.system.levels)
                } catch (e) {
                }
            })
        ...
        // Total sneak attak dice
        {
            ...
            classes.forEach(obj => {
                try {
                    if (obj.system.minionGroup == null || obj.system.minionGroup === "")
                        return;
        ...
        for (let spellbookKey of Object.keys(getProperty(source, "data.attributes.spells.spellbooks"))) {
            ...
            if (classLevel > getProperty(source, `data.classes.${spellbookClass}.maxLevel`))
                classLevel = getProperty(source, `data.classes.${spellbookClass}.maxLevel`);
            const classProgression = getProperty(source, `data.classes.${spellbookClass}.spellPerLevel${classLevel}`);
            linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.spellcastingAbilityBonus`, 0);
            for (let a = 0; a < 10; a++) {
                linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.bonus`, 0);
                const classBase = classProgression !== undefined ? parseInt(classProgression[a + 1]) : -1;
                if (classBase >= 0) {
                    const value = classBase;
                    linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.classBase`, value);
                } else {
                    linkData(source, updateData, `data.attributes.spells.spellbooks.${spellbookKey}.spells.spell${a}.classBase`, 0);
                }
        ...
        linkData(source, updateData, "system.attributes.cmb.total", updateData["data.attributes.bab.total"] - data1.attributes.energyDrain);
        linkData(source, updateData, "system.attributes.cmd.total", 10 + updateData["data.attributes.bab.total"] - data1.attributes.energyDrain);
        linkData(source, updateData, "system.attributes.cmd.flatFootedTotal", 10 + updateData["data.attributes.bab.total"] - data1.attributes.energyDrain);

        // Reset initiative
        linkData(source, updateData, "system.attributes.init.total", 0);

        // Reset class skills
        for (let [k, s] of Object.entries(getProperty(source, "data.skills"))) {
            if (!s) continue;
            const isClassSkill = classes.reduce((cur, o) => {
                if ((getProperty(o.system,"classSkills") || {})[k] === true) return true;
                return cur;
            }, false);
            linkData(source, updateData, `data.skills.${k}.cs`, isClassSkill);
            for (let k2 of Object.keys(getProperty(s, "subSkills") || {})) {
                linkData(source, updateData, `data.skills.${k}.subSkills.${k2}.cs`, isClassSkill);
            }
        }
        if (this.actor.isCharacterType) {
            ...
            if (true) {
                ...
                let classes = this.actor.items.filter(o => o.type === "class" && getProperty(o, "classType") !== "racial" && o.system.automaticFeatures).sort((a, b) => {
                    return a.sort - b.sort;
                });

                for (let i of classes) {
                    classNames.add([i.name, i.system.levels,i.system.addedAbilities || [],i.system.disabledAbilities || [], i.system.customTag])
                }
            ...
            for (let abilityUid of existingAbilities) {
                if (!addedAbilities.has(abilityUid)) {
                    ...
                    changes.splice(changes.findIndex(change => change.source.item.system.uniqueId === abilityUid), 1)
            ...
            if (idsToRemove.length)
                await this.actor.deleteEmbeddedEntity("Item", idsToRemove, {stopUpdates: true});
    ...
    #addDynamicData(updateData, changes, flags, abilities, source, forceModUpdate = false, changeTarget = null) {
        ...
        for (let a of abilities) {
            ...
            if (changes[`data.abilities.${a}.replace`]) {
                linkData(source, updateData, `data.abilities.${a}.total`, changes[`data.abilities.${a}.replace`] + (changes[`data.abilities.${a}.total`] || 0));
                linkData(source, updateData, `data.abilities.${a}.origTotal`, updateData[`data.abilities.${a}.total`] + (changes[`data.abilities.${a}.total`] || 0));
            } else {
                linkData(source, updateData, `data.abilities.${a}.total`, updateData[`data.abilities.${a}.total`] + (changes[`data.abilities.${a}.total`] || 0));
                linkData(source, updateData, `data.abilities.${a}.origTotal`, updateData[`data.abilities.${a}.total`]);
            }
            if (changes[`data.abilities.${a}.total`]) delete changes[`data.abilities.${a}.total`]; // Remove used mods to prevent doubling
            if (changes[`data.abilities.${a}.replace`]) delete changes[`data.abilities.${a}.replace`]; // Remove used mods to prevent doubling
            linkData(source, updateData, `data.abilities.${a}.mod`, Math.floor((updateData[`data.abilities.${a}.total`] - ablPenalty - 10) / 2));
            linkData(source, updateData, `data.abilities.${a}.mod`, Math.max(-5, updateData[`data.abilities.${a}.mod`]));
            linkData(source, updateData, `data.abilities.${a}.origMod`, Math.floor((updateData[`data.abilities.${a}.origTotal`] - ablPenalty - 10) / 2));
            linkData(source, updateData, `data.abilities.${a}.origMod`, Math.max(-5, updateData[`data.abilities.${a}.origMod`]));
            linkData(source, updateData, `data.abilities.${a}.drain`, updateData[`data.abilities.${a}.drain`] + (changes[`data.abilities.${a}.drain`] || 0));
        ...
        linkData(source, updateData, "system.attributes.acp.total", Math.max(updateData["data.attributes.acp.gear"], updateData["data.attributes.acp.encumbrance"]));
        linkData(source, updateData, "system.attributes.maxDex.total", Math.min(updateData["data.attributes.maxDex.gear"] == null ? 999 : updateData["data.attributes.maxDex.gear"], updateData["data.attributes.maxDex.encumbrance"] || 999));
        linkData(source, updateData, "system.attributes.maxDexBonus", Math.min(updateData["data.attributes.maxDex.gear"] == null ? 999 : updateData["data.attributes.maxDex.gear"], updateData["data.attributes.maxDex.encumbrance"] || 999));


        // Force speed to creature speed
        for (let speedKey of Object.keys(getProperty(this.actor.system,"attributes.speed"))) {
            if (changes[`data.attributes.speed.${speedKey}.replace`])
                linkData(source, updateData, `data.attributes.speed.${speedKey}.total`, changes[`data.attributes.speed.${speedKey}.replace`]);
        }
        if (changes[`data.attributes.bab.replace`]) {
            linkData(source, updateData, `data.attributes.bab.total`, changes[`data.attributes.bab.replace`]);
            linkData(source, updateData, `data.attributes.cmb.total`, changes[`data.attributes.bab.replace`]);
        }

        // Add ability mods to CMB and CMD
        const cmbMod = Object.keys(CONFIG.D35E.actorSizes).indexOf(getProperty(source, "data.traits.size") || "") <= Object.keys(CONFIG.D35E.actorSizes).indexOf("tiny") ? modDiffs["str"] : modDiffs["str"];
        linkData(source, updateData, "system.attributes.cmb.total", updateData["data.attributes.cmb.total"] + cmbMod);
        linkData(source, updateData, "system.attributes.cmd.total", updateData["data.attributes.cmd.total"] + modDiffs["str"]);
        if (!flags.loseDexToAC || modDiffs["dex"] < 0) {
            linkData(source, updateData, "system.attributes.cmd.total", updateData["data.attributes.cmd.total"] + modDiffs["dex"]);
            linkData(source, updateData, "system.attributes.cmd.flatFootedTotal", updateData["data.attributes.cmd.flatFootedTotal"] + Math.min(0, modDiffs["dex"]));
        }
        linkData(source, updateData, "system.attributes.cmd.flatFootedTotal", updateData["data.attributes.cmd.flatFootedTotal"] + modDiffs["str"]);

        // Add dex mod to initiative
        linkData(source, updateData, "system.attributes.init.total", updateData["data.attributes.init.total"] + modDiffs["dex"]);

        // Add ability mods to saving throws
        for (let [s, a] of Object.entries(CONFIG.D35E.savingThrowMods)) {
            linkData(source, updateData, `data.attributes.savingThrows.${s}.total`, updateData[`data.attributes.savingThrows.${s}.total`] + modDiffs[a]);
        }
        // Apply changes
        for (let [changeTarget, value] of Object.entries(changes)) {
        ...
        if (changes[`data.attributes.savingThrows.fort.replace`]) {
            linkData(source, updateData, `data.attributes.savingThrows.fort.total`, changes[`data.attributes.savingThrows.fort.replace`]);
        }
        if (changes[`data.attributes.savingThrows.ref.replace`]) {
            linkData(source, updateData, `data.attributes.savingThrows.fort.total`, changes[`data.attributes.savingThrows.ref.replace`]);
        }
        if (changes[`data.attributes.savingThrows.will.replace`]) {
            linkData(source, updateData, `data.attributes.savingThrows.fort.total`, changes[`data.attributes.savingThrows.will.replace`]);
        }

        if (changes["data.attributes.ac.flatFooted.replace"]) {
            linkData(source, updateData, "system.attributes.ac.flatFooted.total", changes["data.attributes.ac.flatFooted.replace"]);
        }
        if (changes["data.attributes.ac.touch.replace"]) {
            linkData(source, updateData, "system.attributes.ac.touch.total", changes["data.attributes.ac.touch.replace"]);
        }
        if (changes["data.attributes.ac.normal.replace"]) {
            linkData(source, updateData, "system.attributes.ac.normal.total", changes["data.attributes.ac.normal.replace"]);
        }


        if (changes[`data.attributes.hp.replace`])
            linkData(source, updateData, `data.attributes.hp.max`, changes[`data.attributes.hp.replace`]);
    ...
    #updateSkills(updateData, data) {
        ...
        for (let [sklKey, skl] of Object.entries(systemData.skills)) {
            ...
            let ablMod = 0;
            if (skl.ability !== "")
                ablMod = systemData.abilities[skl.ability].mod;
            let specificSkillBonus = skl.changeBonus || 0;

            // Parse main skills
            let cs = skl.cs;
            if (systemData.details.levelUpData && systemData.details.levelUpProgression)
                cs = true;
            ...
            for (let [subSklKey, subSkl] of Object.entries(skl.subSkills || {})) {
                ...
                ablMod = 0
                if (subSkl.ability !== "")
                    ablMod = subSkl.ability ? systemData.abilities[subSkl.ability].mod : 0;
    ...
    async #addDefaultChanges(source, changes, flags, sourceInfo, fullConditions, sizeOverride, options = {}, updateData) {
        ...
        const racialHD = source.items.filter(o => o.type === "class" && getProperty(o.system, "classType") === "racial").sort((a, b) => {
            return a.sort - b.sort;
        });
        ...
        const manual_health = (health_source) => {
            let health = health_source.system.hp + (health_source.system.classType === "base") * health_source.system.fc.hp.value;
        ...
        const compute_health = (health_sources, options) => {
            ...
            if (options.auto) {
                let maximized = 0;
                try {
                    maximized = new Roll35e(`${options.maximized || "0"}`, { totalHD: source.system.attributes.hd.total, sourceHD: typeHD }).rollSync().total
                } catch {
                    maximized = 1;
                }
                for (const hd of health_sources) {
        ...
        // Natural armor
        {
            const natAC = getProperty(source, "data.attributes.naturalAC") || 0;
            if (natAC > 0) {
                changes.push({
        ...
        const flyKey = getProperty(source, "data.attributes.speed.fly.maneuverability");
        let flyValue = 0;
        ...
        {
            const climbSpeed = getProperty(source, "data.attributes.speed.climb.total") || 0;
            const swimSpeed = getProperty(source, "data.attributes.speed.swim.total") || 0;
            if (climbSpeed > 0) {
                changes.push({
        ...
        if (!options.skipToken && tokenSizeKey !== 'none' && this.actor.isCharacterType && !getProperty(this.actor.system,"noTokenOverride"))
        {
        ...
        if (!options.skipToken && this.actor.isCharacterType)
        {
            ...
            let lowLight = getProperty(source, "data.attributes.senses.lowLight") !== undefined ?  getProperty(source, "data.attributes.senses.lowLight") : getProperty(this.actor.system,"attributes.senses.lowLight") || false;
            let lowLightMultiplier = getProperty(source, "data.attributes.senses.lowLightMultiplier") !== undefined ?  getProperty(source, "data.attributes.senses.lowLightMultiplier") : getProperty(this.actor.system,"attributes.senses.lowLightMultiplier") || 2;
            let darkvision = getProperty(source, "data.attributes.senses.darkvision") !== undefined ?  getProperty(source, "data.attributes.senses.darkvision") :  getProperty(this.actor.system,"attributes.senses.darkvision") || 0;
            ...
            for (let i of this.actor.items.values()) {
                ...
                if (i.system.equipped && !i.system.melded && !i.broken) {
                    ...
                    lowLightMultiplier = lowLightMultiplier < (i.system.senses?.lowLightMultiplier || 2) ? (i.system.senses?.lowLightMultiplier || 2) : lowLightMultiplier;
            ...
            if (!getProperty(this.actor.system,"noLightOverride") && !game.settings.get("D35E", "globalDisableTokenLight")) {
                if (this.actor.isToken) {
                    ...
                    tokens.push(this.actor.token);
                    for (const o of tokens) {
                        await this.actor.updateTokenLight(dimLight, o, brightLight, color, animationIntensity, type, animationSpeed, lightAngle, alpha);
                    }
                }
                if (!this.actor.isToken) {
                    let tokens = this.actor.getActiveTokens().filter(o => o?.actorLink);
                    for (const o of tokens) {
                        this.actor.updateTokenLight(dimLight, o, brightLight, color, animationIntensity, type, animationSpeed, lightAngle, alpha);
                    }
                }
            }
            if (!getProperty(this.actor.system,"noVisionOverride") && !game.settings.get("D35E", "globalDisableTokenVision"))
            {
    ...
    #getCarryCapacity(srcData) {
        ...
        const carryStr = (srcData.system.abilities.str.total - srcData.system.abilities.str.damage) + srcData.system.abilities.str.carryBonus;
    ...
    getReducedMovementSpeed(srcData1, value, updateData, armorItems, flags, speedKey) {
        ...
        if (load / maxLoad > 2.0 && !flags.noEncumbrance) {
            ...
            speed = 5;
            maxRun = 0;
        } else if ((armorItems.filter(o => getProperty(o.system, "equipmentSubtype") === "heavyArmor" && o.system.equipped && !o.system.melded && !o.broken).length && !flags.heavyArmorFullSpeed) || ((3.0 * load / maxLoad) > 2.0 && !flags.noEncumbrance)) {
            speed = reduceMaxSpeedFromEncumbrance(maxSpeed)
            maxRun = Math.max(1,3+runSpeedMultiplierModifier)* speed
        } else if ((armorItems.filter(o => getProperty(o.system, "equipmentSubtype") === "mediumArmor" && o.system.equipped && !o.system.melded && !o.broken).length && !flags.mediumArmorFullSpeed) || ((3.0 * load / maxLoad) > 1.0 && !flags.noEncumbrance)) {
            speed = reduceMaxSpeedFromEncumbrance(maxSpeed)
