export class TokenDocumentPF extends TokenDocument {
    ...
    refreshDetectionModes() {
        ...
        // Prepare sight
        const darkvisionRange = this.actor?.system?.attributes?.senses?.darkvision ?? 0;
        if (darkvisionRange > 0) {
            this.sight.range = this.actor?.system?.attributes?.senses?.darkvision;
        ...
        if (!seeInvMode && (this.actor?.system?.attributes?.senses?.seeInvisible || this.actor?.system?.attributes?.senses?.truesight)) {
            this.detectionModes.push({ id: seeInvId, enabled: true, range: this.actor?.system?.attributes?.senses?.seeInvisible || this.actor?.system?.attributes?.senses?.truesight });
        ...
        if (!blindSightMode && this.actor?.system?.attributes?.senses?.blindsight) {
            this.detectionModes.push({ id: blindSightId, enabled: true, range: this.actor.system.attributes.senses.blindsight });
        } else if (blindSightMode != null) {
            if (!this.actor?.system?.attributes?.senses?.blindsight) {
        ...
        if (!blindSightMode && this.actor?.system?.attributes?.senses?.tremorsense) {
            this.detectionModes.push({ id: tremorSenseId, enabled: true, range: this.actor.system.attributes.senses.tremorsense });
        } else if (tremorSenseMode != null) {
            if (!this.actor?.system?.attributes?.senses?.tremorsense) {
