...
export class ItemCharges {
    ...
    async addCharges(value, data = null) {
        ...
        if (getProperty(chargeItem.system, "uses.per") === "single"
            && getProperty(chargeItem.system, "quantity") == null) return;

        if (this.item.type === "card") return this.#addCardCharges(value, data);
        if (this.item.type === "spell") return this.#addSpellUses(value, data);

        let prevValue = this.item.isSingleUse ? getProperty(chargeItem.system, "quantity") : getProperty(chargeItem.system, "uses.value");
        if (data != null && this.item.isSingleUse && data["data.quantity"] != null) prevValue = data["data.quantity"];
        else if (data != null && !this.item.isSingleUse && data["data.uses.value"] != null) prevValue = data["data.uses.value"];
    ...
    async #addSpellUses(value, data = null) {
        ...
        const spellbook = getProperty(this.item.actor.system, `attributes.spells.spellbooks.${this.item.system.spellbook}`),
            ...
            spellLevel = getProperty(this.item.system, "level");
        const newCharges = usePowerPoints ? Math.max(0, (getProperty(spellbook, `powerPoints`) || 0) + value * getProperty(this.item.system, "powerPointsCost")) : isSpontaneous
